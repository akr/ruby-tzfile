...
But this means results may incorrect.
...
=end
  ...
  def TZFile.parse(input, visitor)
    magic, version, reserved, ttisgmtcnt, ttisstdcnt, leapcnt, timecnt, typecnt, charcnt =
      input.read(44).unpack('a4 C a15 NNNNNN');
...
=begin
== methods:
Following methods can be used as class methods of a class generated by
TZFile.create because TZFile extends the class.
...
--- gm(sec, min, hour, mday, mon, year, wday, yday, isdst, zone)
    ...
    even with same broken-down time.
=end
  def utc(*args)
    if args.length == 10
...
--- each_timetype {|timetype| ...}
    Evaluate the block for each ((<time type|TZFile::TimeType>)).
=end
  def each_timetype
    @timetype.each {|tt|
      yield tt
...
=end
    ...
    def initialize(time=self.class.now.to_i)
      ...
      when Integer 
        ...
        raise TypeError.new("can't treat as a time: #{time.inspect}")
      end
      @utc = false
...
=end
	...
	return @localutc_data if @localutc_data
      ...
      tt = self.class.timetype_withleap(@time)
      if utc
	gmtoff = 0
	zone_abbrev = 'UTC'
	zone_gmtoff = '+00:00'
...
--- strftime(format)
    ...
    Note that this method is locale independent - always POSIX locale style.
    `E' and `O' modifier is accepted but ignored.
=end
    def strftime(format)
      return format.gsub(/%[EO]?([%aAbBcCdDeFgGhHIjmMnprRStTuUVwWxXyYzZ])/) {
        case $1
	when '%'; '%'
	...
	when 'F'; strftime('%Y-%m-%d')
	when 'g'; sprintf('%02d', current_data.date.cwyear % 100)
	when 'G'; sprintf('%d', current_data.date.cwyear)
	when 'h'; strftime('%b')
	...
	when 'z'; sprintf("%+03d%02d", *(gmtoff / 60).divmod(60)) 
...
=end
    def gmtoff; return current_data.tt.gmtoff; end
